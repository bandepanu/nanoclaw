...
function buildMounts(
  ...
  // the sessDir mount above; we write the creds file there so the container
  ...
  if (fs.existsSync(gcalcliDir)) {
    mounts.push({ hostPath: gcalcliDir, containerPath: '/home/node/.gcalcli', readonly: false });
  }
  const gcalcliDataDir = path.join(homeDir, '.local', 'share', 'gcalcli');
  if (fs.existsSync(gcalcliDataDir)) {
    mounts.push({ hostPath: gcalcliDataDir, containerPath: '/home/node/.local/share/gcalcli', readonly: false });
  }

  // gmail-mcp — OAuth tokens for Gmail MCP server
  const gmailDir = path.join(homeDir, '.gmail-mcp');
  if (fs.existsSync(gmailDir)) {
    mounts.push({ hostPath: gmailDir, containerPath: '/home/node/.gmail-mcp', readonly: false });
  }

  // gdrive-mcp — OAuth tokens for Google Drive MCP server
...
async function buildContainerArgs(
  ...
  // are routed through the agent vault for credential injection. When
  // ONECLI_URL is not configured (OAuth auth mode on this VPS), skip
  // gracefully — credentials come from the OAuth credential copy in buildMounts().
  if (ONECLI_URL) {
    if (agentIdentifier) {
    ...
    log.info('OneCLI URL not configured — using OAuth credential copy mode', { containerName });
  }
  ...
  // Composio API key + MCP URL — inject if configured on host
  const homeDir = os.homedir();
  const composioKeyFile = path.join(homeDir, '.composio', 'api.key');
  ...
  const composioUrlFile = path.join(homeDir, '.composio', 'mcp.url');
